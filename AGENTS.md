# AGENTS.md — mdns-browser-snap

Repackages the prebuilt upstream `mdns-browser` Linux binary
(`mdns-browser_linux_x64`) as a strictly-confined snap. The defining feature
of this repo is that the snap **build verifies the upstream binary's SLSA
build provenance** before packaging it, so a compromised upstream publish step
...
- `gh attestation verify <binary> --repo hrzlgnm/mdns-browser --bundle
...
upstream asset, runs `gh attestation download` + `gh attestation
